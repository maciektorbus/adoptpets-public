...
<body>
    ...
    @endforeach
		...
		<div class="px-4 py-5 my-5 text-center">
			<h1 class="display-5 fw-bold">{{ __('messages.appname') }}</h1>
			<div class="col-lg-6 mx-auto">
				...
				<div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
					<a href="{{ route('petslisting') }}" class="btn btn-primary btn-lg px-4 gap-3">{{ __('messages.browse') }}</a>
					@auth
					<a href="{{ route('donate') }}" class="btn btn-outline-secondary btn-lg px-4">{{ __('messages.donate') }}</a>
					@endauth
				</div>
			</div>
		</div>
		...
		<div class="row">
			@foreach($instagramphotos as $instagramphoto => $instagramlink)
			<div class="col-md-4">
				<div class="embed-responsive embed-responsive-16by9" style="width: 18rem;">
					<img class="card-img-top embed-responsive-item" src="{{ $instagramphoto }}" alt="Card image cap">
				</div>
				<a href="{{ url($instagramlink) }}" class="btn btn-outline-primary me-2 mt-3 mb-3" role="button">{{ __('messages.instagram') }}</a>
			</div>
			@endforeach
			</div>
		</div>
