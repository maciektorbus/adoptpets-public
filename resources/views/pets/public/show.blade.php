	...
	<main>
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col">
					<p>Age: {{ $pet['age'] }}</p>
					<p>Sex: {{ $pet->sex == 1 ? 'Male' : 'Female' }}</p>
					<p>{{ $pet['description'] }}</p>
				</div>
			</div>
			<p>Liked this pet? Adopt by reaching out to us!</p>
			<a href="{{ route('contact') }}" class="btn btn-primary btn-lg px-4 gap-3">{{ __('messages.contact') }}</a>
			@auth
			<p>Want to show some support? Donate!</p>
